...
def plot_data3d(d, setlimits=None):
    ...
    frame1.plot(d[:, 0], d[:, 1], d[:, 2], markersize=2,
                marker='x', alpha=0.1,
                zorder=1, color='k', linestyle='none')
    frame2.plot(d[:, 3], d[:, 4], markersize=2,
                marker='x', alpha=0.1,
                zorder=1, color='k', linestyle='none')
    ...
    for it in range(len(unique_labels)):
        ...
        if k_it != -1:
            frame1.plot(xy[:, 0], xy[:, 1], xy[:, 2], markersize=2,
                        ...
                        linestyle='none')
            frame2.plot(xy[:, 3], xy[:, 4], markersize=2,
                        marker=marks[it], alpha=alpha,
                        zorder=zorder, color=tuple(colors[it]),
                        linestyle='none')

            stats = get_5d_stats(xy, stats)

        else:
            frame1.plot(xy[:, 0], xy[:, 1], xy[:, 2], markersize=1,
                        marker='.', zorder=zorder, color='k',
                        linestyle='none')
            frame2.plot(xy[:, 3], xy[:, 4], markersize=1,
                        marker='.', zorder=zorder, color='k',
                        linestyle='none')
...
if __name__ == "__main__":
    ...
    data = np.array([normdata[COLNAMES[0]], normdata[COLNAMES[1]],
                     normdata[COLNAMES[2]], normdata[COLNAMES[3]],
                     normdata[COLNAMES[4]], normdata[COLNAMES[5]]]).T
